@echo off
rem ****
rem ****---- Compilation for CFL_to_CIF Program ----****
rem ****
rem > INIT
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c cfl2cif.f90   /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:cfl2cif *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000
   )
rem
   if [%_COMP%]==[gfortran] (
      gfortran -c cfl2cif.f90           %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o cfl2cif_gf.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem
