...
rem   
   if [%_COMP%]==[gfortran] (
      if [%_DEBUG%]==[Y] (
         ...
         if [%_VER%]==[m32] (set DIRECTORY=gfortran) else (set DIRECTORY=gfortran64_release)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c Similar.f90   /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:Similar *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000
      copy Similar.exe %FULLPROF%\.      
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c Similar.f90           %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o Similar_gf.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
      copy Similar_gf.exe %FULLPROF%\.      
   )
rem   
