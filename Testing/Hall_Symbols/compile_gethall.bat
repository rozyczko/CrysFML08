@echo off
rem -----------------------------------------
rem ---- Compilation for GetHall Program ----
rem -----------------------------------------
rem > INIT 
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c GetHall_from_Gener.f90  /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:GetHall *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000 
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c GetHall_from_Gener.f90           %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o GetHall.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem   
