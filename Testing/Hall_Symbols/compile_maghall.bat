@echo off
rem -----------------------------------------
rem ---- Compilation for MagHall Program ----
rem -----------------------------------------
rem > INIT 
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c Magnetic_Hall.f90  /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:MHall *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:256000000 
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c Magnetic_Hall.f90           %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
