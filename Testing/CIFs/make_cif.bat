@echo off
rem ****
rem ****---- Compilation for CIF Program ----****
rem ****
rem > INIT 
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c cif.f90           /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:cif *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:150000000 
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c cif.f90             %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o cif_gf.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem   
