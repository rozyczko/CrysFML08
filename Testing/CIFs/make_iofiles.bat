...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c io_files.f90   /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:io_files *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:3000000000
      copy io_files.exe %FULLPROF%\.      
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c io_files.f90           %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o io_files_gf.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
