...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c read_ssg_database.f90                    /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:read_ssg_database *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000 
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c read_ssg_database.f90                    %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o read_ssg_database.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem   
