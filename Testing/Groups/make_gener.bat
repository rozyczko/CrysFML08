...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c Generators-Symbol.f90                    /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC 
      ifort /exe:Generators.exe *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000
   )
rem
   if [%_COMP%]==[gfortran] (
      gfortran -c Generators-Symbol.f90                    %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC -I\.
      gfortran -o Generators_gf.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem
