@echo off
rem ****
rem ****---- Compilation for Test_SSG_Genk Program ----****
rem ****
rem > INIT
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c Test_SSG_Genk.f90                    /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:ssg_genk.exe *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000
   )
rem
   if [%_COMP%]==[gfortran] (
      gfortran -c Test_SSG_Genk.f90                    %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC -I\.
      gfortran -o ssg_genk_gf.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem
