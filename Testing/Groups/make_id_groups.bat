...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c Id_groups.f90   /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:Id_groups *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000
   )
rem
   if [%_COMP%]==[gfortran] (
      gfortran -c Id_groups.f90           %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o Id_groups.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem
