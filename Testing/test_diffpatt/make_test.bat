@echo off
rem ****
rem ****---- Compilation for Test_Read Program ----****
rem ****
rem > INIT 
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c test_read.f90                  /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:test_read *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000 
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c test_read.f90                    %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o test_read.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem 
