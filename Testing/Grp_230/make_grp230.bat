...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c groups_230.f90  /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC
      ifort /exe:Grp230 *.obj  %CRYSFML%\%DIRECTORY%\LibC\crysfml.lib /link /stack:300000000 
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c groups_230.f90          %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC
      gfortran -o Grp230.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC -lcrysfml
   )
rem   
