  Module Input_output_data_mod
    ...
    ! Global variables
    real(kind=cp),                 public :: step, norm_mon, fwh0_f=0.03,fwh1_f=0.00,fwh2_f=0.00, eta0_f=0.0,eta1_f=0.0, &
    ...
    character(len=20),             public :: spg_symb,algor="CURFIT"
    ...
    Subroutine Read_CFL(cfl_file)
      ...
      !-----
      integer                          :: i,j, i_cfl, ier, nlines
      character(len=20)                :: keyw
      character(Len=180)               :: texte
      character(Len=180),dimension(40) :: line
    ...
    contains
     ...
     Subroutine set_initial_conditions(nscan)
       ...
       do i=1,NPEAKX
         ...
         weak=.false.
         vs%pv(j+1)= max(0.1*(d%y(k)-averb),0.01)           !Intensity, a factor 10 lower than peak intensity
         Intens(i)=vs%pv(j+1)
         !if(d%y(k) < max_int) weak=.true.
         if(Intens(i) < 5.0) weak=.true.
         if(.not. weak) vs%code(j+1)=1 !Intensities are always refined
  ...
  Program rl_Int1D
    ...
    Do ! repeat for all qscans in the file !
      ...
      if(texte(1:5) == "QSCAN") then
         ...
         !do i=1,npeakx
         !   if(satellite(i)) then
         !      write(*,"(i3,tr4,5i4,4f12.5,tr2,L3)") i, hkl(:3+nkvec,i),peak_pos(i), Intens(i),fwhms(i),etas(i),satellite(i)
         !   else
         !      write(*,"(i3,tr4,5i4,4f12.5,tr2,L3)") i, hkl(:3+nkvec,i),peak_pos(i), Intens(i),fwhmf(i),etaf(i),satellite(i)
         !   end if
         !end do
         ...
         DO i=1,npeakx
           write(unit=i_out,fmt="(2f14.6,f14.2,2F14.6,tr5,4i3)") corr_pp(i),vs%pv(j+1), vs%pv(j),vs%pv(j+2),vs%pv(j+3),   &
                                       vs%code(j+1), vs%code(j),vs%code(j+2),vs%code(j+3)
           j=j+4
         END DO
         ...
         do i=1,npeakx
           ...
           if(satellite(i)) then
             fwh=fwhms(i)
             sfwh=sqrt(Vs%sPv(7)**2 + (pos*Vs%sPv(8))**2 + (pos*pos*Vs%sPv(9))**2 + vs%spv(j+2)**2)
           else
             fwh=fwhmf(i)
             sfwh=sqrt(Vs%sPv(4)**2 + (pos*Vs%sPv(5))**2 + (pos*pos*Vs%sPv(6))**2 + vs%spv(j+2)**2)
           end if
           write(unit=line,fmt="(a,3f9.4,6f12.4)") trim(testchi2)//"  hkl, Int, Sig, FWHM, Sig: ",&
              rhkl,vs%pv(j+1),vs%spv(j+1),fwh,sfwh
