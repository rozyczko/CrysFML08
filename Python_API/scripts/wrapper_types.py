...
write_list_to_array(f,t : dict,s : str,lucy : dict,alloc : bool= True) -> None
write_list_to_array2d(f,t : dict,s : str,lucy : dict,alloc : bool= True) -> None
write_list_to_array_class(f,t : dict,s : str,lucy : dict) -> None
...
def wrap_cfml_module_types(m : cfml_objects.Module,m_name : str,lucy : dict) -> None:
    ...
    with open(w_file,'w') as f:
        ...
        for s in t:
            ...
            write_list_to_array(f,t,s,lucy,alloc=False)
            write_list_to_array2d(f,t,s,lucy)
            write_list_to_array2d(f,t,s,lucy,alloc=False)
            if not t[s].parent:
                write_unwrap_proc(f,t,s,lucy)
                if t[s].childs:
                    write_unwrap_proc_no_alloc(f,t,s,lucy)
                    write_list_to_array_class(f,t,s,lucy)
    ...
    with open(w_file,'w') as f:
        ...
        for p in publics_wrap:
            ...
            if n == 0:
                ...
                f.write(f",wrap_{p[0]}")
            n += 1
        f.write(f"\n{'':>4}public :: list_to_array,list_to_array_no_alloc,list_to_array_class\n")
        f.write(f"\n{'':>4}interface list_to_array\n")        
        f.write(f"{'':>8}module procedure list_to_array_character\n")     
        f.write(f"{'':>8}module procedure list_to_array2d_character\n")
        f.write(f"{'':>8}module procedure list_to_array_logical\n")
        f.write(f"{'':>8}module procedure list_to_array2d_logical\n")
        for m in modules:
            t = modules[m].types
            for s in t:
                f.write(f"{'':>8}module procedure list_to_array_{s.lower()}\n")
                f.write(f"{'':>8}module procedure list_to_array2d_{s.lower()}\n")
        f.write(f"{'':>4}end interface\n")
        f.write(f"\n{'':>4}interface list_to_array_no_alloc\n")        
        f.write(f"{'':>8}module procedure list_to_array_character_no_alloc\n")        
        f.write(f"{'':>8}module procedure list_to_array2d_character_no_alloc\n")
        f.write(f"{'':>8}module procedure list_to_array_logical_no_alloc\n")
        f.write(f"{'':>8}module procedure list_to_array2d_logical_no_alloc\n")
        for m in modules:
            t = modules[m].types
            for s in t:
                f.write(f"{'':>8}module procedure list_to_array_{s.lower()}_no_alloc\n")
                f.write(f"{'':>8}module procedure list_to_array2d_{s.lower()}_no_alloc\n")
        f.write(f"{'':>4}end interface\n")
        f.write(f"\n{'':>4}interface list_to_array_class\n")
        for m in modules:
            t = modules[m].types
            for s in t:
                if not t[s].parent and t[s].childs:
                    f.write(f"{'':>8}module procedure list_to_array_{s.lower()}_class\n")
        f.write(f"{'':>4}end interface\n")
        f.write(f"\n{'':>4}interface\n")
        for p in publics_unwrap:
        ...
        for m in modules:
            ...
            for s in t:
                if not t[s].parent and t[s].childs:
                    f.write(f"\n{'':>8}module subroutine list_to_array_{s.lower()}_class(procedure_name,var_name,my_list,arr,ierror)\n")
                    ...
                    f.write(f"{'':>8}end subroutine list_to_array_{s.lower()}_class\n")        
        ...
        f.write(f"\n{'':>8}module subroutine list_to_array2d_character(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
        f.write(f"{'':>12}character(len=*), dimension(:,:), allocatable, intent(out) :: arr\n")
        f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
        f.write(f"{'':>8}end subroutine list_to_array2d_character\n")
        ...
        f.write(f"\n{'':>8}module subroutine list_to_array2d_logical(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
        f.write(f"{'':>12}logical, dimension(:,:), allocatable, intent(out) :: arr\n")
        f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
        f.write(f"{'':>8}end subroutine list_to_array2d_logical\n")
        for m in modules:
            t = modules[m].types
            for s in t:                
                ...
                f.write(f"\n{'':>8}module subroutine list_to_array2d_{s.lower()}(procedure_name,var_name,my_list,arr,ierror)\n")
                f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
                f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
                f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
                f.write(f"{'':>12}type({s}), dimension(:,:), allocatable, intent(out) :: arr\n")
                f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
                f.write(f"{'':>8}end subroutine list_to_array2d_{s.lower()}\n")
        f.write(f"\n{'':>8}module subroutine list_to_array_character_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
        f.write(f"{'':>12}character(len=*), dimension(:), intent(out) :: arr\n")
        f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
        f.write(f"{'':>8}end subroutine list_to_array_character_no_alloc\n")
        f.write(f"\n{'':>8}module subroutine list_to_array2d_character_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
        f.write(f"{'':>12}character(len=*), dimension(:,:), intent(out) :: arr\n")
        f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
        f.write(f"{'':>8}end subroutine list_to_array2d_character_no_alloc\n")
        f.write(f"\n{'':>8}module subroutine list_to_array_logical_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
        f.write(f"{'':>12}logical, dimension(:), intent(out) :: arr\n")
        f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
        f.write(f"{'':>8}end subroutine list_to_array_logical_no_alloc\n")
        f.write(f"\n{'':>8}module subroutine list_to_array2d_logical_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
        f.write(f"{'':>12}logical, dimension(:,:), intent(out) :: arr\n")
        f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
        f.write(f"{'':>8}end subroutine list_to_array2d_logical_no_alloc\n")
        for m in modules:
            t = modules[m].types
            for s in t:                
                f.write(f"\n{'':>8}module subroutine list_to_array_{s.lower()}_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
                f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
                f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
                f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
                f.write(f"{'':>12}type({s}), dimension(:), intent(out) :: arr\n")
                f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
                f.write(f"{'':>8}end subroutine list_to_array_{s.lower()}_no_alloc\n")
                f.write(f"\n{'':>8}module subroutine list_to_array2d_{s.lower()}_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
                f.write(f"{'':>12}character(len=*), intent(in) :: procedure_name\n")
                f.write(f"{'':>12}character(len=*), intent(in) :: var_name\n")
                f.write(f"{'':>12}type(list), intent(inout) :: my_list\n")
                f.write(f"{'':>12}type({s}), dimension(:,:), intent(out) :: arr\n")
                f.write(f"{'':>12}integer, intent(inout) :: ierror\n")
                f.write(f"{'':>8}end subroutine list_to_array2d_{s.lower()}_no_alloc\n")
        f.write(f"\n{'':>4}end interface\n")        
        f.write(f"\nEnd Module CFML_Wraps\n")

def write_list_to_array(f,t : dict,s : str,lucy : dict,alloc : bool= True) -> None:

    if alloc:
        f.write(f"\n{'':>4}Module Subroutine list_to_array_{s}(procedure_name,var_name,my_list,arr,ierror)\n")
    else:
        f.write(f"\n{'':>4}Module Subroutine list_to_array_{s}_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
    # Arguments
    f.write(f"\n{'':>8}! Arguments\n")
    ...
    if not alloc:
        ...
        f.write(f"{'':>12}err_cfml%msg  = 'list_to_array_{s}_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>8}end if\n")
    ...
    if alloc:
        ...
        f.write(f"\n{'':>4}End Subroutine list_to_array_{s}_no_alloc\n")

def write_list_to_array2d(f,t : dict,s : str,lucy : dict,alloc : bool= True) -> None:

    if alloc:
        f.write(f"\n{'':>4}Module Subroutine list_to_array2d_{s}(procedure_name,var_name,my_list,arr,ierror)\n")
    else:
        f.write(f"\n{'':>4}Module Subroutine list_to_array2d_{s}_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
    # Arguments
    f.write(f"\n{'':>8}! Arguments\n")
    f.write(f"{'':>8}character(len=*), intent(in) :: procedure_name\n")
    f.write(f"{'':>8}character(len=*), intent(in) :: var_name\n")
    ...
    if not alloc:
        ...
        f.write(f"{'':>12}err_cfml%flag = .true.\n")
        f.write(f"{'':>12}err_cfml%ierr = -1\n")
        f.write(f"{'':>12}err_cfml%msg  = 'list_to_array2d_{s}_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>8}end if\n")
    ...
    if not alloc:
        ...
        f.write(f"{'':>24}err_cfml%msg  = 'list_to_array2d_{s}_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>20}end if\n")
    else:
    ...
    f.write(f"{'':>24}if (ierror == 0) call unwrap_{s}_no_alloc(my_dict,arr(i+1,j+1),ierror)\n")
    ...
    f.write(f"{'':>8}end if\n")
    if alloc:
        f.write(f"\n{'':>4}End Subroutine list_to_array2d_{s}\n")
    else:
        f.write(f"\n{'':>4}End Subroutine list_to_array2d_{s}_no_alloc\n")

def write_list_to_array_class(f,t : dict,s : str,lucy : dict) -> None:

    f.write(f"\n{'':>4}Module Subroutine list_to_array_{s}_class(procedure_name,var_name,my_list,arr,ierror)\n")
    # Arguments
    f.write(f"\n{'':>8}! Arguments\n")
    f.write(f"{'':>8}character(len=*), intent(in) :: procedure_name\n")
    f.write(f"{'':>8}character(len=*), intent(in) :: var_name\n")
    ...
    f.write(f"{'':>24}err_cfml%flag = .true.\n")
    f.write(f"{'':>24}err_cfml%ierr = -1\n")
    f.write(f"{'':>24}err_cfml%msg  = 'list_to_array_{s}_class: Cannot determine fortran type'\n")
    ...
    f.write(f"{'':>24}err_cfml%flag = .true.\n")
    f.write(f"{'':>24}err_cfml%ierr = -1\n")
    f.write(f"{'':>24}err_cfml%msg  = 'list_to_array_{s}_class: Wrong fortran type'\n")
    f.write(f"{'':>20}end if\n")
    f.write(f"{'':>16}end if\n")
    f.write(f"{'':>16}if (ierror == 0) call unwrap_{s}_no_alloc(my_dict,arr(i+1),ierror)\n")
    f.write(f"{'':>16}if (ierror == 0) ierror = err_cfml%ierr\n")
    f.write(f"{'':>12}end do\n")
    f.write(f"{'':>8}end if\n")
    f.write(f"\n{'':>4}End Subroutine list_to_array_{s}_class\n")
    ...
    for c in t.components:
        ...
        if var.ndim == 0:
            if var.ftype.lower() in PRIMITIVES:
                ...
                f.write(f"{tab}if (ierror == 0) call unwrap_dict_item('Unwrap_{t.name}','{var.name}',py_var,di_{var.name},ierror)\n")
                f.write(f"{tab}if (ierror == 0) call unwrap_{lucy[var.ftype]}(di_{var.name},{forvar}%{var.name},ierror)\n")
        else:
            if var.ftype.lower() in NUMERICALS:
                if var.ftype.lower() == 'integer':
                ...
                if var.is_class:
                    f.write(f"{tab}if (ierror == 0) call list_to_array_class('Unwrap_{t.name}','{var.name}',my_list,{forvar}%{var.name},ierror)\n")
                else:
                    if var.allocatable:
                        f.write(f"{tab}if (ierror == 0) call list_to_array('Unwrap_{t.name}','{var.name}',my_list,{forvar}%{var.name},ierror)\n")
                    else:
                        f.write(f"{tab}if (ierror == 0) call list_to_array_no_alloc('Unwrap_{t.name}','{var.name}',my_list,{forvar}%{var.name},ierror)\n")
...
def write_wraps_primitives():
    ...
    with open(w_file,'w') as f:
        ...
        f.write(f"\n{'':>4}subroutine list_to_array2d_character(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"\n{'':>8}! Arguments\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>8}type(list), intent(inout) :: my_list\n")
        ...
        f.write(f"{'':>20}do j = 0 , n-1\n")
        f.write(f"{'':>24}if (ierror == 0) ierror = li%getitem(item,j)\n")
        f.write(f"{'':>24}if (ierror == 0) ierror = cast(my_str,item)\n")
        f.write(f"{'':>24}if (ierror == 0) arr(i+1,j+1) = my_str\n")
        f.write(f"{'':>24}if (ierror == 0) ierror = err_cfml%ierr\n")
        f.write(f"{'':>20}end do\n")
        f.write(f"{'':>16}end do\n")
        f.write(f"{'':>12}end if\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"\n{'':>4}end subroutine list_to_array2d_character\n")
        f.write(f"\n{'':>4}subroutine list_to_array_character_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"\n{'':>8}! Arguments\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>8}type(list), intent(inout) :: my_list\n")
        f.write(f"{'':>8}character(len=*), dimension(:), intent(out) :: arr\n")
        f.write(f"{'':>8}integer, intent(inout) :: ierror\n")
        f.write(f"\n{'':>8}! Local variables\n")
        f.write(f"{'':>8}integer :: i,n\n")
        f.write(f"{'':>8}character(len=:), allocatable :: my_str\n")
        ...
        f.write(f"{'':>12}err_cfml%flag = .true.\n")
        f.write(f"{'':>12}err_cfml%ierr = -1\n")
        f.write(f"{'':>12}err_cfml%msg  = 'list_to_array_character_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"{'':>8}if (n > 0) then\n")
        f.write(f"{'':>12}do i = 0 , n-1\n")
        f.write(f"{'':>16}if (ierror == 0) ierror = my_list%getitem(item,i)\n")
        ...
        f.write(f"{'':>12}end do\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"\n{'':>4}end subroutine list_to_array_character_no_alloc\n")     
        f.write(f"\n{'':>4}subroutine list_to_array2d_character_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"\n{'':>8}! Arguments\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>8}type(list), intent(inout) :: my_list\n")
        ...
        f.write(f"{'':>12}err_cfml%flag = .true.\n")
        f.write(f"{'':>12}err_cfml%ierr = -1\n")
        f.write(f"{'':>12}err_cfml%msg  = 'list_to_array_character2d_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"{'':>8}if (n > 0) then\n")
        f.write(f"{'':>12}do i = 0 , m-1\n")
        f.write(f"{'':>16}if (ierror == 0) ierror = my_list%getitem(item,i)\n")
        f.write(f"{'':>16}if (ierror == 0) ierror = cast(li,item)\n")
        f.write(f"{'':>16}ierror = li%len(n)\n")
        f.write(f"{'':>16}if (n /= size(arr,2)) then\n")
        f.write(f"{'':>20}ierror = -1\n")
        f.write(f"{'':>20}err_cfml%flag = .true.\n")
        f.write(f"{'':>20}err_cfml%ierr = -1\n")
        f.write(f"{'':>20}err_cfml%msg  = 'list_to_array_character2d_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>16}end if\n")
        f.write(f"{'':>16}do j = 0 , n-1\n")
        f.write(f"{'':>20}if (ierror == 0) ierror = li%getitem(item,j)\n")
        f.write(f"{'':>20}if (ierror == 0) ierror = cast(my_str,item)\n")
        f.write(f"{'':>20}if (ierror == 0) arr(i+1,j+1) = my_str\n")
        f.write(f"{'':>20}if (ierror == 0) ierror = err_cfml%ierr\n")
        f.write(f"{'':>16}end do\n")
        f.write(f"{'':>12}end do\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"\n{'':>4}end subroutine list_to_array2d_character_no_alloc\n")
        ...
        f.write(f"\n{'':>4}subroutine list_to_array2d_logical(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"\n{'':>8}! Arguments\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>8}type(list), intent(inout) :: my_list\n")
        ...
        f.write(f"{'':>20}do j = 0 , n-1\n")
        f.write(f"{'':>24}if (ierror == 0) ierror = li%getitem(item,j)\n")
        f.write(f"{'':>24}if (ierror == 0) ierror = cast(arr(i+1,j+1),item)\n")
        f.write(f"{'':>24}if (ierror == 0) ierror = err_cfml%ierr\n")
        f.write(f"{'':>20}end do\n")
        f.write(f"{'':>16}end do\n")
        f.write(f"{'':>12}end if\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"\n{'':>4}end subroutine list_to_array2d_logical\n")
        f.write(f"\n{'':>4}subroutine list_to_array_logical_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"\n{'':>8}! Arguments\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>8}type(list), intent(inout) :: my_list\n")
        f.write(f"{'':>8}logical, dimension(:), intent(out) :: arr\n")
        f.write(f"{'':>8}integer, intent(inout) :: ierror\n")
        f.write(f"\n{'':>8}! Local variables\n")
        f.write(f"{'':>8}integer :: i,n\n")
        f.write(f"{'':>8}type(object) :: item\n")
        ...
        f.write(f"{'':>12}err_cfml%flag = .true.\n")
        f.write(f"{'':>12}err_cfml%ierr = -1\n")
        f.write(f"{'':>12}err_cfml%msg  = 'list_to_array_logical_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"{'':>8}if (ierror == 0 .and. n > 0) then\n")
        f.write(f"{'':>12}allocate(arr(n))\n")
        f.write(f"{'':>12}do i = 0 , n-1\n")
        ...
        f.write(f"{'':>12}end do\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"\n{'':>4}end subroutine list_to_array_logical_no_alloc\n")
        f.write(f"\n{'':>4}subroutine list_to_array2d_logical_no_alloc(procedure_name,var_name,my_list,arr,ierror)\n")
        f.write(f"\n{'':>8}! Arguments\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: procedure_name\n")
        f.write(f"{'':>8}character(len=*), intent(in) :: var_name\n")
        f.write(f"{'':>8}type(list), intent(inout) :: my_list\n")
        ...
        f.write(f"{'':>12}err_cfml%flag = .true.\n")
        f.write(f"{'':>12}err_cfml%ierr = -1\n")
        f.write(f"{'':>12}err_cfml%msg  = 'list_to_array2d_logical_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"{'':>8}if (n > 0) then\n")
        f.write(f"{'':>12}do i = 0 , m-1\n")
        f.write(f"{'':>16}if (ierror == 0) ierror = my_list%getitem(item,i)\n")
        ...
        f.write(f"{'':>20}err_cfml%flag = .true.\n")
        f.write(f"{'':>20}err_cfml%ierr = -1\n")
        f.write(f"{'':>20}err_cfml%msg  = 'list_to_array2d_logical_no_alloc: Dimension of list and arr inconsistent'\n")
        f.write(f"{'':>16}end if\n")
        f.write(f"{'':>16}do j = 0 , n-1\n")
        f.write(f"{'':>20}if (ierror == 0) ierror = li%getitem(item,j)\n")
        f.write(f"{'':>20}if (ierror == 0) ierror = cast(arr(i+1,j+1),item)\n")
        f.write(f"{'':>20}if (ierror == 0) ierror = err_cfml%ierr\n")
        f.write(f"{'':>16}end do\n")
        f.write(f"{'':>12}end do\n")
        f.write(f"{'':>8}end if\n")
        f.write(f"\n{'':>4}end subroutine list_to_array2d_logical_no_alloc\n")
        f.write(f"\nend submodule")
    ...
    for c in t.components:
        ...
        if var.ndim == 0:
            if var.ftype.lower() in PRIMITIVES:
                ...
                f.write(f"{tab}if (ierror == 0) call wrap_{lucy[var.ftype]}({forvar}%{var.name},di_{var.name},ierror)\n")
            ...
            if var.ftype.lower() in NUMERICALS:
                ...
                if var.ndim == 1:
                    ...
                    f.write(f"{tab}        if (ierror == 0) call wrap_{lucy[var.ftype]}({forvar}%{var.name}(i),di_{var.name}(i),ierror)\n")
                    ...
                    f.write(f"{tab}            if (ierror == 0) call wrap_{lucy[var.ftype]}({forvar}%{var.name}(i,j),di_{var.name}(i,j),ierror)\n")
