@echo off
rem ****
rem ****---- Compilation for Read_SSG_Database Program ----****
rem ****
rem > INIT 
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c read_ssg_database.f90                    /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC08
      ifort /exe:read_ssg_database *.obj  %CRYSFML%\%DIRECTORY%\LibC08\crysfml.lib /link /stack:300000000 
   )
rem   
   if [%_COMP%]==[gfortran] (
      gfortran -c read_ssg_database.f90                    %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC08
      gfortran -o read_ssg_database.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC08 -lcrysfml
   )
rem   
