@echo off
rem ****
rem ****---- Compilation for Generators-Symbol Program ----****
rem ****
rem > INIT
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c Generators-Symbol.f90                    /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC08
      ifort /exe:Generators.exe *.obj  %CRYSFML%\%DIRECTORY%\LibC08\crysfml.lib /link /stack:300000000
   )
rem
   if [%_COMP%]==[gfortran] (
      gfortran -c Generators-Symbol.f90                    %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC08 -I\.
      gfortran -o Generators_gf.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC08 -lcrysfml
   )
rem
