@echo off
rem ****
rem ****---- Compilation for Id_Groups Program ----****
rem ****
rem > INIT
   (set _DEBUG=N)
   (set _COMP=ifort)
...
rem > Compilation
   if [%_COMP%]==[ifort] (
      ifort /c Id_groups.f90   /nologo %OPT1% /I%CRYSFML%\%DIRECTORY%\LibC08
      ifort /exe:Id_groups *.obj  %CRYSFML%\%DIRECTORY%\LibC08\crysfml.lib /link /stack:300000000
   )
rem
   if [%_COMP%]==[gfortran] (
      gfortran -c Id_groups.f90           %OPT1% -I%CRYSFML%\%DIRECTORY%\LibC08
      gfortran -o Id_groups.exe *.o -L%CRYSFML%\%DIRECTORY%\LibC08 -lcrysfml
   )
rem
