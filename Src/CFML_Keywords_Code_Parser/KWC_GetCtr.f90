Submodule (CFML_Keywords_Code_Parser) KWC_GetCtr
   !---- Variables ----!
   implicit none
 ...
 Contains
    ...
    Module Subroutine Get_Atombet_Ctr(X,Betas,Spgr,Codini,Icodes,Multip,Ord,Ss,Ipr)
       ...
       if (order > 1 ) then
          do j=2,order
             Rsym=Spgr%OP(ss_ptr(j))%Mat(1:3,1:3)  !SymOp(ss_ptr(j))%Rot
             Rs=real(Rsym)
    ...
    Module Subroutine Get_Atompos_Ctr(X,Spgr,Codini,ICodes,Multip,Ord,Ss,Att,Ipr)
       ...
       if(order > 1 ) then  !A constraint in position must exist
          ...
          do k=2,order
             symbol=" "
             Rsym=Spgr%OP(ss_ptr(k))%Mat(1:3,1:3)                !Spgr%SymOp(ss_ptr(k))%Rot
             tr=Spgr%OP(ss_ptr(k))%Mat(1:3,4)
             tr=tr + atr(:,k)                                 !tr=Spgr%SymOp(ss_ptr(k))%tr + atr(:,k)
             call Get_SymSymb(Rsym,tr,Sym_Symb)
             tsymbol= Symmetry_Symbol(Rsym,tr)                            !symmetry_symbol(Sym_Symb)
             i=index(tsymbol,";")
             if(i /= 0) then
